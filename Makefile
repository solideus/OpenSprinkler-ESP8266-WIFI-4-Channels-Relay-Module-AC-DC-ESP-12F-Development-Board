...
.PHONY: test-api
test-api: test-board-profiles test-esp8266-4ch-relay test-hardware-detection test-storage-files test-sprinkler-log test-firmware-release test-buffer-filler test-string-buffer test-sensor-units test-weather-sensor-cache test-weather-failsafe test-output-sequencer test-bundle-codec test-w5500-frame test-flow-rate-window
	$(MAKE) clean
...
.PHONY: test-esp8266-4ch-relay
test-esp8266-4ch-relay:
	@set -e; output=$$(mktemp); trap 'rm -f "$$output"' EXIT; \
		$(CXX) -std=gnu++14 -I. tests/esp8266_4ch_relay_test.cpp core/output_sequencer.cpp -o "$$output"; \
		"$$output"
