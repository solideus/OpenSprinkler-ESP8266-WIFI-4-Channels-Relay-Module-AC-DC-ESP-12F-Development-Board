...
void OpenSprinkler::begin() {
...
#if defined(ESP8266)
	Wire.begin(); // init I2C
	#if defined(ESP8266_4CH_RELAY)
		// This board drives four relays directly and has no OpenSprinkler
		// station-driver I/O expander. Keep all outputs off while booting.
		hw_rev = 1;
		hw_type = HW_TYPE_UNKNOWN;
		PIN_BUTTON_1 = RELAY4_PIN_BUTTON_1;
		PIN_BUTTON_2 = RELAY4_PIN_BUTTON_2;
		PIN_BUTTON_3 = RELAY4_PIN_BUTTON_3;
		PIN_SENSOR1 = RELAY4_PIN_SENSOR_1;
		PIN_SENSOR2 = RELAY4_PIN_SENSOR_2;

		const uint8_t station_pins[] = {
			RELAY4_PIN_STATION_1, RELAY4_PIN_STATION_2,
			RELAY4_PIN_STATION_3, RELAY4_PIN_STATION_4
		};
		for(uint8_t i = 0; i < sizeof(station_pins); i++) {
...
#if defined(ESP8266)
	#if defined(ESP8266_4CH_RELAY)
		status.has_curr_sense = 0;  // this relay board has no current-sense circuit
		baseline_current = 0;
	#else
	...
	pinModeExt(PIN_BUTTON_1, INPUT_PULLUP);
	#if defined(ESP8266_4CH_RELAY)
		// GPIO15 uses the external pull-down required for normal ESP8266 boot.
...
void OpenSprinkler::apply_all_station_bits(void (*post_activation_callback)()) {
		...
		for(uint8_t i = 0; i < sizeof(station_pins); i++) {
			const bool on = status.enabled && (station_bits[0] & (1U << i));
			digitalWrite(station_pins[i], on ? HIGH : LOW);
		}
	#else
...
/** Button functions */
bool OpenSprinkler::button_is_pressed(unsigned char pin_butt) {
#if defined(ESP8266_4CH_RELAY)
	// B2 is wired to 3.3 V and is active HIGH; B1/B3 are active LOW.
	return digitalReadExt(pin_butt) == ((pin_butt == PIN_BUTTON_2) ? HIGH : LOW);
#else
	return digitalReadExt(pin_butt) == LOW;
#endif
}
...
unsigned char OpenSprinkler::button_read_busy(unsigned char pin_butt, unsigned char waitmode, unsigned char butt, unsigned char is_holding) {
	...
	if (waitmode==BUTTON_WAIT_NONE || (waitmode == BUTTON_WAIT_HOLD && is_holding)) {
		if (!button_is_pressed(pin_butt)) return BUTTON_NONE;
		return butt | (is_holding ? BUTTON_FLAG_HOLD : 0);
	}

	while (button_is_pressed(pin_butt) &&
...
unsigned char OpenSprinkler::button_read(unsigned char waitmode)
{
	...
	} else if (button_is_pressed(PIN_BUTTON_2)) {
		curr = button_read_busy(PIN_BUTTON_2, waitmode, BUTTON_2, is_holding);
	} else if (button_is_pressed(PIN_BUTTON_3)) {
