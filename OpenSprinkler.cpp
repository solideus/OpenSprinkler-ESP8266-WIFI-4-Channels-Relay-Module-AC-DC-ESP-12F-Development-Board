...
void OpenSprinkler::begin() {
	...
	Wire.begin(); // init I2C
#if defined(ESP8266_4CH_RELAY)
	osboard::select(osboard::PROFILE_ESP8266_4CH_RELAY);
	hw_rev = 1;
	// No official station driver, strap probe, RF, Ethernet or booster.
	// Preload LOW before switching the four active-HIGH relays to outputs.
	for (uint8_t pin : osboard::RELAY4_STATION_PINS) {
	...
	pinModeExt(PIN_BUTTON_1, INPUT_PULLUP);
	#if defined(ESP8266_4CH_RELAY)
		pinModeExt(PIN_BUTTON_2, INPUT); // GPIO15 has an external pull-down
	#else
	pinModeExt(PIN_BUTTON_2, INPUT_PULLUP);
	#endif
	pinModeExt(PIN_BUTTON_3, INPUT_PULLUP);
...
#if defined(ESP8266_4CH_RELAY)
	// Use the sequenced output, including bundle members and master priority.
	for (uint8_t i = 0; i < osboard::RELAY4_STATION_COUNT; i++) {
		const bool on = osboard::relay4_station_on(applied_station_bits[0], status.enabled, i);
		digitalWrite(osboard::RELAY4_STATION_PINS[i], on ? HIGH : LOW);
	}
...
#if defined(ESP8266_4CH_RELAY)
	return osboard::relay4_button_pressed(pin_butt, digitalReadExt(pin_butt) != LOW);
#else
