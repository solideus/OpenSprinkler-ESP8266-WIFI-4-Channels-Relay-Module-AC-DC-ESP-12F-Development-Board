...
/** This section removed to make way for the GPIO pins of the ESP8266 WIFI 4 Channels Relay Module AC/DC ESP-12F Development Board which are the GPIO16 (D0)GPIO14 (D5)GPIO12 (D6)GPIO13 (D7) pins )*/
...
#else
/** ESP12F_RELAY_X4 Modificación ESP8266 WIFI 4 Channels Relay Module AC/DC ESP-12F Development Board”.*/
...
#endif /** ESP12F_RELAY_X4 End Modification*/
}
...
void OpenSprinkler::lcd_print_time(time_t t)
{
	...
	lcd_print_pgm(PSTR(" "));
	#if(LCD_ALT_DATE_FORMAT == 0) // studioshemp MM-DD - LCD Alt Date Format defined in defines.h 
	...
	if (!status.enabled) {
		...
		for (byte s=0; s<8; s++) {
			...
			if (sid == iopts[IOPT_MASTER_STATION]) {
				...
				lcd.print((bitvalue&1) ? c : '-'); //Originally an _ character but I find the -	makes it easier to read on the OLED display (the _ forms a continuous line, the - clearly seperates zones) /StudioShemp
			}
			bitvalue >>= 1;
		}
	}
...
#if(ESP12F_RELAY_X4 == 0)
/**	ESP12F_RELAY_X4 This section removed (modified below) as button 2 will be high (1) when pressed.  */
...
#else	/**	ESP12F_RELAY_X4 Begin modified section */
...
#endif	/** ESP12F_RELAY_X4 End modified section */	
	}
...
#else		
	} else if (digitalReadExt(PIN_BUTTON_2) == 1) {	/** ESP12F_RELAY_X4  Line modified as GPIO for button 2 will go high when pressed */
#endif
