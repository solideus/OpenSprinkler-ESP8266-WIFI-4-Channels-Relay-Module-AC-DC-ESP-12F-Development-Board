...
/** Date Format Define */ 
#define LCD_ALT_DATE_FORMAT 1  // set to 1 for DD-MM  or 0 for MM-DD  (Sets alternate date format on LCD for those of us who are irked by the US date format - code altered in OpenSprinkler.cpp to accomodate) //StudioShemp

/**ESP12F_RELAY_X4 Define */
#define ESP12F_RELAY_X4 1  // set to 1 if using the ESP12F_RELAY_X4 Board, otherwise set to 0
...
#else
	/* OS30 revision 1 pin defines */ //ESP12_RELAY_X4 GPIO definitions
	// pins on PCA9555A IO expander have pin numbers IOEXP_PIN+i
	#define V1_IO_CONFIG         0x1F00 // config bits
	#define V1_IO_OUTPUT         0x1F00 // output bits
