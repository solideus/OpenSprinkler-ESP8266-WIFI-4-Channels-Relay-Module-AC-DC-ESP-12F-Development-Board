...
int main() {
	...
	check_sensor_pins(PROFILE_ESP8266_4CH_RELAY, 2, 1, 3, UNUSED_PIN, UNUSED_PIN);
	assert(active().pins.buttons[0] == 2);
	assert(active().pins.buttons[1] == 15);
	assert(active().pins.buttons[2] == 0);
	assert(active().pins.rf_rx == UNUSED_PIN);
	assert(active().pins.rf_tx == UNUSED_PIN);
	assert(active().pins.boost == UNUSED_PIN);
	assert(active().pins.boost_enable == UNUSED_PIN);
	assert(active().pins.latch_common == UNUSED_PIN);
	assert(active().pins.latch_common_anode == UNUSED_PIN);
	assert(active().pins.latch_common_cathode == UNUSED_PIN);
	assert(active().pins.io_expander_interrupt == UNUSED_PIN);
	for (uint8_t pin : active().pins.sensor_pullups) assert(pin == UNUSED_PIN);
	assert(!has(CAP_ETHERNET));
